...
class DeviceMonitor:
    ...
    def check_ping(self):
        if self.device.ip is None:
            return False
        try:
            result = icmp_ping(self.device.ip, count=self.device.ping_count,
                               timeout=self.device.ping_timeout, privileged=False)
            return result.packets_received > 0
        except Exception as e:
            if self.previous_status is not False:
                print(
                    f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} : PING ' : exception {e}")
            return False
    ...
    def check_url(self):
        ...
        while i < self.device.http_retry:
            try:
                if i > 0:
                    time.sleep(self.device.http_timeout)
                response = requests.get(
                    url=self.device.url, timeout=self.device.http_timeout, verify=False,)
                ...
                if self.previous_status is not False:
                    print(
                        f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} : HTTP ' : exception {e}")
                i += 1
        else:
            return False
        ...
        while i < self.device.ssh_retry:
            ...
            try:
                ...
                if self.device.ssh_key_file:
                    ssh_key_file_path = Path(
                        ssh_key_folder_path, self.device.ssh_key_file)
                else:
                    ...
                    try:
                        ...
                        if not regex.search(text):
                            if self.previous_status is not False:
                                ...
                                print(
                                    f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} : SSH : not found required regex '{pattern}' Failed")
                            ok = False
                    except re.error:
                        if self.previous_status is not False:
                            print(
                                f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} : SSH : invalid required regex '{pattern}' Ignored")
                for pattern in self.device.ssh_pattern_forbiden:
                    try:
                        ...
                        if regex.search(text):
                            if self.previous_status is not False:
                                print(
                                    f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} : SSH  : found forbidden regex '{pattern}' Failed")
                            ok = False
                    except re.error:
                        if self.previous_status is not False:
                            print(
                                f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} : SSH : invalid forbidden regex '{pattern}' Ignored")
                return ok
            except Exception as e:
                if self.previous_status is not False:
                    print(
                        f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} : SSH : exception {e}")
                i += 1
            finally:
                try:
                    ...
                    if self.previous_status is not False:
                        print(
                            f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} : SSH closing  : exception {e}")
        else:
            return False
    ...
    def check_mount(self):
        ...
        try:
            ...
            if result and self.device.mount_test_file:
                mount_test_file_path = Path(
                    mount_folder_path, self.device.mount_test_file)
                stream = None
                try:
            ...
            if self.previous_status is not False:
                print(
                    f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} : MOUNT : exception {e}")
            return False
    ...
    def run_monitor(self, config: Config):
        ...
        while True:
            ...
            if self.previous_status is None or self.previous_status != ok:
                if ok:
                    downtime_duration = time.time(
                    ) - self.report.downtime_start if self.report.downtime_start else 0
                    if downtime_duration > 0:
                        print(
                            f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} {config.log_message_reconnected} ({config.log_message_unreachable_for} {downtime_duration:.1f}s)")
                    else:
                        print(
                            f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} {config.log_message_reachable}")
                    self.report.downtime_start = None
                else:
                    self.report.downtime_start = time.time()
                    print(
                        f"[{time.strftime('%Y/%m/%d %H:%M:%S')}] {self.device.name} {config.log_message_unreachable}")
